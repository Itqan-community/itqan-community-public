...
الصور في المشاركات كانت ساكنة: لا تكبير ولا عرض بملء الشاشة. صار النقر عليها يفتحها في طبقة كاملة، بإيماءات اللمس المعتادة.

<p align="center">
  <img src="docs/screenshots/lightbox.gif" alt="نقر لفتح الصورة، ثم قرصة للتكبير، وسحب للتنقّل، وسحب لأسفل للإغلاق" width="300">
</p>

- **قرصة** للتكبير حتى ستّة أضعاف، و**سحب** للتنقّل داخل الصورة المكبّرة.
- **نقرتان** للتبديل بين الحجم الطبيعي والمكبّر، مع تثبيت النقطة تحت الإصبع.
- **سحب لأسفل** للإغلاق حين تكون الصورة بحجمها الطبيعي.
- على سطح المكتب: عجلة الفأرة، و`Esc`، والنقر على الخلفية.
- التركيز محصور داخل الطبقة ويعود إلى الصورة عند الإغلاق، وتمرير الصفحة موقوف خلفها.

بلا مكتبة خارجية: المكوّن في `js/src/forum/components/ImageLightbox.js` والربط في `addImageLightbox.js`.

## صفحة الأقسام

الشبكة كانت مثبّتة على أربعة أعمدة بلا أيّ media query، فعلى شاشة ٤١٤ بكسل يصير عرض البطاقة ٩٨ بكسل. صارت الآن `repeat(auto-fill, minmax(264px, 1fr))`: تتكيّف مع أي عرض دون نقاط توقّف مرصوصة يدويًا، وارتفاع البطاقة يتبع محتواها بدل `200px` ثابتة.

![شبكة الأقسام على ثلاثة أعراض، قبل وبعد](docs/screenshots/categories-grid.png)
...
| `less/forum/categories.less` | شبكة بطاقات الأقسام |
| `less/forum/lightbox.less` | طبقة عرض الصور |
